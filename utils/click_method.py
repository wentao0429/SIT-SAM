...
def generate_uniform_click_points(img3D, num_clicks=8):
    # 获取图像的形状
    # print(img3D.shape)
    _, _, depth, height, width = img3D.shape

    # 在每个维度上生成均匀分布的点击点
    z = np.linspace(0, depth - 1, num_clicks)
    y = np.linspace(0, height - 1, num_clicks)
    x = np.linspace(0, width - 1, num_clicks)

    # 使用meshgrid生成3D网格点
    zv, yv, xv = np.meshgrid(z, y, x, indexing='ij')

    # 将3D网格点转换为坐标列表
    click_points = np.round(np.stack([zv, yv, xv], axis=-1).reshape(-1, 1, 3))

    # 创建标签，所有的标签都是正的（值为1）
    click_labels = np.ones((click_points.shape[0], 1), dtype=int)

    # 将numpy数组转换为torch张量
    click_points = torch.from_numpy(click_points).int().unsqueeze(1)
    click_labels = torch.from_numpy(click_labels).int().unsqueeze(1)

    return click_points, click_labels
...
def get_next_click3D_torch_ritm(prev_seg, gt_semantic_seg):
    ...
    for i in range(gt_semantic_seg.shape[0]):
        ...
        if points.nelement() == 0:
            center_point = [dim // 2 for dim in to_point_mask[i].shape]
            point = torch.tensor(center_point)
        else:
