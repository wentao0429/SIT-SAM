...
def interaction(sam_model, classifier, image_embedding, gt3D, num_clicks, click_points, click_labels, click_type,
    ...
    for num_click in range(num_clicks):
        ...
        if num_click == num_clicks - 1:
            ...
            features.clear()
    using_time = s_time + cls_time
    return prev_masks, return_loss, cls_pred, click_points, click_labels, using_time
...
def finetune_model_predict3D(img3D, gt3D, sam_model_tune, classifier, cls_label, device='cuda', click_method='random',
    ...
    with torch.no_grad():
        ...
        click_points = []
        click_labels = []
        prev_masks, loss, cls_pred, click_points, click_labels, using_time = interaction(sam_model_tune, classifier,
                                                                             image_embedding, gt3D,
                                                                             num_clicks, click_points, click_labels,
                                                                             click_method)
        print(f"Time: {encoding_time + using_time} s")
