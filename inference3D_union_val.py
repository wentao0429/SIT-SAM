...
def compute_iou(pred_mask, gt_semantic_seg):
    ...
    iou = np.sum(in_mask, axis=(1, 2, 3, 4)) / np.sum(out_mask, axis=(1, 2, 3, 4))
    return iou
...
def compute_dice(mask_gt, mask_pred):
    ...
    """
    volume_sum = mask_gt.sum(axis=(1, 2, 3, 4)) + mask_pred.sum(axis=(1, 2, 3, 4))
    volume_intersect = (mask_gt & mask_pred).sum(axis=(1, 2, 3, 4))
    return 2 * volume_intersect / (volume_sum + 1e-10)
...
def finetune_model_predict3D(img3D, gt3D, sam_model_tune, device='cuda', click_method='random', num_clicks=10,
    ...
    batch_size = img3D.size(0)  # Get batch size

    click_points = [[] for _ in range(batch_size)]
    click_labels = [[] for _ in range(batch_size)]

    pred_list = [[] for _ in range(batch_size)]  # Create list of lists to store predictions for each sample
    iou_list = [[] for _ in range(batch_size)]
    dice_list = [[] for _ in range(batch_size)]
    ...
    for num_click in range(num_clicks):
        with torch.no_grad():
            # if (num_click > 1):  # 当num_click循环到大于1时，才使用random
            click_method = "random"
            ...
            for i in range(batch_size):
                click_points[i].append(points_co[i])
                click_labels[i].append(points_la[i])

            points_input = points_co
            labels_input = points_la
            # print(batch_points.shape, batch_labels.shape)
            ...
            medsam_seg_prob = torch.sigmoid(prev_masks).cpu().numpy()  # (B, 1, 64, 64, 64)
            # convert prob to mask
            medsam_seg = (medsam_seg_prob > 0.5).astype(np.uint8)
            # medsam_seg等并没有使用batchsize
            iou_vals = compute_iou(medsam_seg, gt3D.detach().cpu().numpy())
            dice_vals = compute_dice(gt3D.detach().cpu().numpy().astype(np.uint8), medsam_seg)
            for i in range(batch_size):
                pred_list[i].append(medsam_seg[i])
                iou_list[i].append(iou_vals[i])
                dice_list[i].append(dice_vals[i])

    return pred_list, click_points, click_labels, iou_list, dice_list


if __name__ == "__main__":
    all_dataset_paths = [os.path.join(args.test_data_path, case) for case in os.listdir(args.test_data_path)]
    ...
    for batch_data in tqdm(test_dataloader):
        ...
        if (args.dim == 3):
            ...
            # all_dice_list.extend(dice_list)
        elif (args.dim == 2):
            seg_mask_list, points, labels, iou_list, dice_list = finetune_model_predict2D(
                ...
                click_method=args.point_method, num_clicks=args.num_clicks,
                prev_masks=None)
        os.makedirs(vis_root, exist_ok=True)                                                                                                 f"_pred{idx}.nii.gz")))

        for batch_idx in range(sz[0]):  
            per_iou = max(iou_list[batch_idx])
            all_iou_list.append(per_iou)
            all_dice_list.append(max(dice_list[batch_idx]))
            print(dice_list[batch_idx])
            out_dice[img_name[batch_idx]] = max(dice_list[batch_idx])
            cur_dice_dict = OrderedDict()
            for i, dice in enumerate(dice_list[batch_idx]):
                cur_dice_dict[f'{i}'] = dice
            out_dice_all[img_name[batch_idx]] = cur_dice_dict
