...
class KNNAttention(nn.Module):
    ...
    def forward(
        ...
        if self.xl_max_memories > 0:
            new_kv_memories_discarded, new_xl_kv_memories = new_kv_memories[:, :-self.xl_max_memories], new_kv_memories[
                                                                                                        :,
                                                                                                        -self.xl_max_memories:]
        else:
        ...
        # preparation to add this to block-recurrent-transformer-pytorch,
        # for the pinnacle of long context attention network
...
class MemorizingTransformer(nn.Module):
    def __init__(
        ...
        memorizing_layers = default(memorizing_layers,
                                    (depth // 2,))  # default KNN attention layer to midpoint of transformer
        ...
        for idx in range(depth):
            ...
            if use_knn_attention:
                attn = KNNAttention(dim=dim, dim_head=dim_head, heads=knn_attn_heads, dropout=attn_dropout,
    ...
    def forward(
        ...
        assert all([memory.num_indices == batch_size for memory in
                    knn_memories]), f'you passed in an input with batch size {batch_size} but your memories were not instantiated with that number of KNN indices'
        ...
        # shifting memories a number of layers down, little known technique shown to enhance memories from Ernie-Doc
        # paper
