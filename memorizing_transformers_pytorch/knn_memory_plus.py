...
# Use a more generic default path
DEFAULT_KNN_MEMORY_MEMMAP_DIRECTORY = './knn_memories'
...
class KNN:
    ...
    def load_index(self, file_path):
        ...
        self.is_trained = True  # Assume loaded index is already trained
...
class KNNMemory:
    def __init__(
        ...
        if os.path.exists(memmap_filename):
            ...
            # Need to update self.db_offsets and other states based on actual situation
        else:
            # If file doesn't exist, create new memmap file
    ...
    def save(self, directory):
        """Save KNNMemory state to specified directory."""
        # Save each KNN index
        if not os.path.exists(directory):
            os.makedirs(directory)
        for i, knn in enumerate(self.knns):
            index_file_path = os.path.join(directory, f'knn_index_{i}.faiss')
            knn.save_index(index_file_path)

        # Save other necessary states like self.db_offsets
        db_offsets_path = os.path.join(directory, 'db_offsets.npy')
        np.save(db_offsets_path, self.db_offsets)

    def load(self, directory):
        """Load KNNMemory state from specified directory."""
        # Load each KNN index
        for i, knn in enumerate(self.knns):
            index_file_path = os.path.join(directory, f'knn_index_{i}.faiss')
            if os.path.exists(index_file_path):
                knn.load_index(index_file_path)

        # Load self.db_offsets
        db_offsets_path = os.path.join(directory, 'db_offsets.npy')
        if os.path.exists(db_offsets_path):
            self.db_offsets = np.load(db_offsets_path)
