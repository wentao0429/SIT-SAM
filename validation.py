...
def finetune_model_predict3D(img3D, gt3D, sam_model_tune, device='cuda', click_method='random', num_clicks=10,
    ...
    flops_encoder = FlopCountAnalysis(sam_model_tune.image_encoder, img3D.to(device))
    total_flops_image_encoder = flops_encoder.total()
    print("Encoding time:", encoding_time)
    prompt_time = 0
    decoder_time = 0
    total_flops_prompt, total_flops_decoder = 0, 0
    for num_click in range(num_clicks):
        with torch.no_grad():
            ...
            flops_prompt = FlopCountAnalysis(
                sam_model_tune.prompt_encoder,
                ([points_input, labels_input], None, low_res_masks.to(device))
            )
            flops_prompt = flops_prompt.total()
            total_flops_prompt += flops_prompt
            flops_decoder = FlopCountAnalysis(
                sam_model_tune.mask_decoder,
                (
                    image_embedding.to(device),  # (B, 384, 64, 64, 64)
                    sam_model_tune.prompt_encoder.get_dense_pe(),  # (1, 384, 64, 64, 64)
                    sparse_embeddings,  # (B, 2, 384)
                    dense_embeddings,  # (B, 384, 64, 64, 64)
                    False,
                )
            )
            flops_decoder = flops_decoder.total()
            total_flops_decoder += flops_decoder
            start = time.time()
    ...
    print("Inference time:", decoder_time + prompt_time + encoding_time)
    total_flops = total_flops_image_encoder + total_flops_prompt + total_flops_decoder
    print(f"total flops: {total_flops / 1e9:.1f}G")
    return pred_list, click_points, click_labels, iou_list, dice_list, nsd_list
...
if __name__ == "__main__":
    ...
    num_grad_params = count_parameters_grad(sam_model_tune)
    print(f"Number of parameters: {num_params}")
    print(f"Number of grad parameters: {num_grad_params}")
    ...
    for batch_data in tqdm(test_dataloader):
        ...
        if os.path.exists(pred_path):
            ...
            if args.dim == 3:
                seg_mask_list, points, labels, iou_list, dice_list, nsd_list = finetune_model_predict3D(
