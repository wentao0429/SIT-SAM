...
class BaseTrainer:
    ...
    def train_epoch(self, epoch, num_clicks):
        epoch_loss = 0
        epoch_iou = 0  # TODO: 没在train_epoch中使用
        epoch_dice = 0
        self.model.train()
        if self.args.multi_gpu:
            sam_model = self.model.module
            ...
            sam_model = self.model
            self.args.rank = -1  # 表示当前不在多GPU环境下运行。在后续的代码中被用于判断是否需要进行多GPU的相关操作，如同步梯度等
        ...
        step_loss = 0
        for step, (image3D, gt3D, _) in enumerate(tbar):
            ...
            with my_context():
                ...
                loss /= self.args.accumulation_steps  # 累积梯度
            ...
            if not self.args.multi_gpu or (self.args.multi_gpu and self.args.rank == 0):
                if step % self.args.accumulation_steps == 0 and step != 0:
                    ...
                    # self.writer.add_scalar(f'Step Dice/train/Epoch_{epoch}', print_dice, step)
        ...
        epoch_loss /= step
        epoch_dice = epoch_dice * self.args.accumulation_steps / step  # 有些偏差,但不多
